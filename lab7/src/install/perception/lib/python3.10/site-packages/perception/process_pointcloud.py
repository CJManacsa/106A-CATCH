...
class RealSensePCSubscriber(Node):
    def __init__(self):
        ...
        self.declare_parameter('plane.b', -100.0)
        self.declare_parameter('plane.c', 3.0)
        self.declare_parameter('plane.d', 6.0)
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        points_in_range = points[norms <= self.max_distance] 
        above_below = (
            self.a * points_in_range[:, 0] +
            self.b * points_in_range[:, 1] +
            self.c * points_in_range[:, 2] +
            self.d
        )

        # Apply other filtering relative to plane
        # filtered_points = points_in_range
        filtered_points = points_in_range[above_below > 0]
