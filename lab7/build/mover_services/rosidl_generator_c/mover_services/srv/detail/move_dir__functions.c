// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from mover_services:srv/MoveDir.idl
// generated code does not contain a copyright notice
#include "mover_services/srv/detail/move_dir__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `direction`
#include "rosidl_runtime_c/string_functions.h"

bool
mover_services__srv__MoveDir_Request__init(mover_services__srv__MoveDir_Request * msg)
{
  if (!msg) {
    return false;
  }
  // direction
  if (!rosidl_runtime_c__String__init(&msg->direction)) {
    mover_services__srv__MoveDir_Request__fini(msg);
    return false;
  }
  // distance
  return true;
}

void
mover_services__srv__MoveDir_Request__fini(mover_services__srv__MoveDir_Request * msg)
{
  if (!msg) {
    return;
  }
  // direction
  rosidl_runtime_c__String__fini(&msg->direction);
  // distance
}

bool
mover_services__srv__MoveDir_Request__are_equal(const mover_services__srv__MoveDir_Request * lhs, const mover_services__srv__MoveDir_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // direction
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->direction), &(rhs->direction)))
  {
    return false;
  }
  // distance
  if (lhs->distance != rhs->distance) {
    return false;
  }
  return true;
}

bool
mover_services__srv__MoveDir_Request__copy(
  const mover_services__srv__MoveDir_Request * input,
  mover_services__srv__MoveDir_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // direction
  if (!rosidl_runtime_c__String__copy(
      &(input->direction), &(output->direction)))
  {
    return false;
  }
  // distance
  output->distance = input->distance;
  return true;
}

mover_services__srv__MoveDir_Request *
mover_services__srv__MoveDir_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mover_services__srv__MoveDir_Request * msg = (mover_services__srv__MoveDir_Request *)allocator.allocate(sizeof(mover_services__srv__MoveDir_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(mover_services__srv__MoveDir_Request));
  bool success = mover_services__srv__MoveDir_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
mover_services__srv__MoveDir_Request__destroy(mover_services__srv__MoveDir_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    mover_services__srv__MoveDir_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
mover_services__srv__MoveDir_Request__Sequence__init(mover_services__srv__MoveDir_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mover_services__srv__MoveDir_Request * data = NULL;

  if (size) {
    data = (mover_services__srv__MoveDir_Request *)allocator.zero_allocate(size, sizeof(mover_services__srv__MoveDir_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = mover_services__srv__MoveDir_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        mover_services__srv__MoveDir_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
mover_services__srv__MoveDir_Request__Sequence__fini(mover_services__srv__MoveDir_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      mover_services__srv__MoveDir_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

mover_services__srv__MoveDir_Request__Sequence *
mover_services__srv__MoveDir_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mover_services__srv__MoveDir_Request__Sequence * array = (mover_services__srv__MoveDir_Request__Sequence *)allocator.allocate(sizeof(mover_services__srv__MoveDir_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = mover_services__srv__MoveDir_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
mover_services__srv__MoveDir_Request__Sequence__destroy(mover_services__srv__MoveDir_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    mover_services__srv__MoveDir_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
mover_services__srv__MoveDir_Request__Sequence__are_equal(const mover_services__srv__MoveDir_Request__Sequence * lhs, const mover_services__srv__MoveDir_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!mover_services__srv__MoveDir_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
mover_services__srv__MoveDir_Request__Sequence__copy(
  const mover_services__srv__MoveDir_Request__Sequence * input,
  mover_services__srv__MoveDir_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(mover_services__srv__MoveDir_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    mover_services__srv__MoveDir_Request * data =
      (mover_services__srv__MoveDir_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mover_services__srv__MoveDir_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          mover_services__srv__MoveDir_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!mover_services__srv__MoveDir_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
mover_services__srv__MoveDir_Response__init(mover_services__srv__MoveDir_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    mover_services__srv__MoveDir_Response__fini(msg);
    return false;
  }
  return true;
}

void
mover_services__srv__MoveDir_Response__fini(mover_services__srv__MoveDir_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
  // message
  rosidl_runtime_c__String__fini(&msg->message);
}

bool
mover_services__srv__MoveDir_Response__are_equal(const mover_services__srv__MoveDir_Response * lhs, const mover_services__srv__MoveDir_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  return true;
}

bool
mover_services__srv__MoveDir_Response__copy(
  const mover_services__srv__MoveDir_Response * input,
  mover_services__srv__MoveDir_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  return true;
}

mover_services__srv__MoveDir_Response *
mover_services__srv__MoveDir_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mover_services__srv__MoveDir_Response * msg = (mover_services__srv__MoveDir_Response *)allocator.allocate(sizeof(mover_services__srv__MoveDir_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(mover_services__srv__MoveDir_Response));
  bool success = mover_services__srv__MoveDir_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
mover_services__srv__MoveDir_Response__destroy(mover_services__srv__MoveDir_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    mover_services__srv__MoveDir_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
mover_services__srv__MoveDir_Response__Sequence__init(mover_services__srv__MoveDir_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mover_services__srv__MoveDir_Response * data = NULL;

  if (size) {
    data = (mover_services__srv__MoveDir_Response *)allocator.zero_allocate(size, sizeof(mover_services__srv__MoveDir_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = mover_services__srv__MoveDir_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        mover_services__srv__MoveDir_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
mover_services__srv__MoveDir_Response__Sequence__fini(mover_services__srv__MoveDir_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      mover_services__srv__MoveDir_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

mover_services__srv__MoveDir_Response__Sequence *
mover_services__srv__MoveDir_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mover_services__srv__MoveDir_Response__Sequence * array = (mover_services__srv__MoveDir_Response__Sequence *)allocator.allocate(sizeof(mover_services__srv__MoveDir_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = mover_services__srv__MoveDir_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
mover_services__srv__MoveDir_Response__Sequence__destroy(mover_services__srv__MoveDir_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    mover_services__srv__MoveDir_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
mover_services__srv__MoveDir_Response__Sequence__are_equal(const mover_services__srv__MoveDir_Response__Sequence * lhs, const mover_services__srv__MoveDir_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!mover_services__srv__MoveDir_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
mover_services__srv__MoveDir_Response__Sequence__copy(
  const mover_services__srv__MoveDir_Response__Sequence * input,
  mover_services__srv__MoveDir_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(mover_services__srv__MoveDir_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    mover_services__srv__MoveDir_Response * data =
      (mover_services__srv__MoveDir_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mover_services__srv__MoveDir_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          mover_services__srv__MoveDir_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!mover_services__srv__MoveDir_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
