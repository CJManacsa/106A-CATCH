...
class UR7e_CubeGrasp(Node):
    def __init__(self):
        ...
        self.cube_pub = self.create_publisher(PointStamped, '/cube_pose_coord', 10)
    ...
    def transform_cube_pose(self, msg: PointStamped):
        ...
        try:
            ...
            rclpy.time.Time(),   # For the latest transform
            timeout=rclpy.duration.Duration(seconds=1.0)
            )
            # transforms from the msg's child frame which is camera optical frame --> base link
