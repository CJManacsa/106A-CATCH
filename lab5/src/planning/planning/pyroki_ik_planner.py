...
FIXED: High rest_cost weight to prevent large joint movements
"""
...
class PyRokiIKPlanner(Node):
    ...
    def _solve_ik_pyroki(self, target_wxyz: np.ndarray, target_position: np.ndarray) -> np.ndarray:
        ...
        # Build SE3 target properly (FIXED)
    ...
    def _solve_ik_jax(
        ...
        FIXED: Uses MUCH higher rest_cost weight to stay close to current configuration
        
        This uses:
        - pose_cost: Standard pose matching cost (no batch issues)
        - limit_cost: Keeps joints within limits
        - rest_cost: STRONG bias toward current configuration (prevents big jumps)
        - LeastSquaresProblem with Levenberg-Marquardt solver
        """
        # FIXED: Seed with current config if available
        if current_config is not None:
            seed = current_config.flatten()
            joint_var = robot.joint_var_cls(seed)
        else:
            joint_var = robot.joint_var_cls(0)
        
        # Define cost factors
        factors = [
            # CRITICAL FIX: Reduced position weight from 50.0 to 10.0
            # This makes the solver prioritize staying close over exact position
            pk.costs.pose_cost(
                robot,
                joint_var,
                target_pose=T_world_target,
                target_link_index=target_link_index,
                pos_weight=10.0,   # REDUCED from 50.0
                ori_weight=1.0,    # REDUCED from 10.0 (orientation matters less)
            ),
            pk.costs.limit_cost(
                robot,
                joint_var,
        ...
        # Add MASSIVE rest cost if current config provided
        if current_config is not None:
            factors.append(
                pk.costs.rest_cost(
                    joint_var,
                    rest_pose=current_config.flatten(),
                    weight=100.0,  # MASSIVELY INCREASED from 0.1 to 100.0
                    # This is now 10x more important than position accuracy!
                    # Solver will find a nearby solution rather than jumping far away
                )
            )
    ...
    def compute_ik_with_current_state(self, current_joint_state, x, y, z, 
        ...
        """
        # The current_joint_state is now being tracked via subscription
        # and used automatically in the solver via rest_cost
